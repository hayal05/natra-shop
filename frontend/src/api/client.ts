 ...
 *
 * Task 103: `body` may also be a `FormData` instance (used by
 * `uploadThumbnail()`'s multipart file upload) — in that case it's
 * passed straight to `fetch()` untouched and no `Content-Type` header
 * is set, so the browser can set its own `multipart/form-data;
 * boundary=...` value, which can't be replicated by hand. Every other
 * `body` is still JSON-encoded exactly as before.
 */
