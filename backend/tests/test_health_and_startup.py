...
Task 98 extends this with `GET /health/object-storage` (Task 91),
following the exact same `monkeypatch main.check_object_storage`
pattern as `GET /health/playwright`'s two tests just above it —
`check_object_storage()`'s own branches (missing config, OCI
`ServiceError`, unexpected exception) get their real, non-mocked
coverage in the new `test_object_storage.py` instead, since testing
them here would mean either faking Object Storage at the HTTP-route
level (duplicating `test_object_storage.py`'s work) or letting a real
`get_client()` call reach a real, unconfigured OCI SDK client, which
would be a network-dependent test of the exact kind this file's own
`check_connection`/`check_browser` tests already deliberately avoid.
...
# --- GET /health/object-storage (Task 91, tested at Task 98) ------------


def test_health_object_storage_ready(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """
    Endpoint-level check that `health_check_object_storage()` calls
    `check_object_storage()` and merges its result into the same
    `{"service": ...}` envelope every other `/health*` route uses.
    `check_object_storage()`'s own branches (missing config, OCI
    `ServiceError`, unexpected exception) are covered directly against
    the real function in `test_object_storage.py` — this file only
    needs to prove the route itself is wired correctly, the same
    division `test_health_db_connected`/`test_health_playwright_ready`
    already use for their own underlying `check_*` functions.
    """
    from app import main

    monkeypatch.setattr(main, "check_object_storage", lambda: {"object_storage_ready": True})
    resp = client.get("/health/object-storage")
    assert resp.status_code == 200
    assert resp.json() == {"service": "natra-backend", "object_storage_ready": True}


def test_health_object_storage_not_ready_degrades_to_200(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Same never-raises contract as the other two `/health*` checks — see `object_storage.check_object_storage()`'s own docstring."""
    from app import main

    monkeypatch.setattr(
        main,
        "check_object_storage",
        lambda: {"object_storage_ready": False, "error": "Missing required environment variable: OCI_USER_OCID"},
    )
    resp = client.get("/health/object-storage")
    assert resp.status_code == 200
    body = resp.json()
    assert body["object_storage_ready"] is False
    assert body["error"] == "Missing required environment variable: OCI_USER_OCID"
