...
Task 97 updates `_select_products_mine()`'s returned row shape from
five columns to six (`... drive_link, thumbnail_ref`), matching that
task's real query change to `list_my_products()` in `app/main.py`.
`ProductRow` already had a `thumbnail_ref` field (added earlier, for
the buyer-facing grid/detail queries) — only the seller-dashboard
query's returned tuple needed the new column.

Task 100 adds the two query shapes `POST /products/{product_id}/
thumbnail` (Task 94) issues that no earlier task's suite exercised:
a bare `SELECT RAWTOHEX(seller_id) FROM products WHERE id =
HEXTORAW(:product_id)` ownership lookup (distinct from Task 75's
`price, RAWTOHEX(seller_id) ...` — that one's own match string already
takes priority in `execute()`'s dispatch, so the two never collide),
and a real `UPDATE products SET thumbnail_ref = ...`. Both are plain
functions of the existing `ProductRow`/`self._store.products` — no new
row type or store field needed, since `thumbnail_ref` already existed
on `ProductRow`.
...
class FakeCursor:
    ...
    def execute(self, sql: str, **kwargs: Any) -> None:
        ...
        elif "RAWTOHEX(seller_id) FROM products WHERE id = HEXTORAW(:product_id)" in s:
            # Task 100 — upload_product_thumbnail()'s ownership check.
            # Deliberately distinct from `_select_product_price_and_seller`
            # above (that one's real query leads with `price,` before
            # `RAWTOHEX(seller_id)`, so the two column lists never
            # collide) — this is the bare "just the owner" lookup Task 94
            # added.
            self._select_product_seller_id(**kwargs)
        elif "UPDATE products SET thumbnail_ref" in s:
            # Task 100 — upload_product_thumbnail()'s post-upload write.
            self._update_product_thumbnail_ref(**kwargs)
        elif (
    ...
    def _select_products_mine(self, seller_id: str) -> None:
        ...
        self._result = [
            (p.id_hex, p.name, p.price, p.description, p.drive_link, p.thumbnail_ref)
            for p in rows
        ]
    ...
    def _select_product_seller_id(self, product_id: str) -> None:
        # Task 100 — upload_product_thumbnail()'s ownership check. Empty
        # result for an unknown id, matching every other `products`
        # select's "no row" shape.
        row = self._store.products.get(product_id)
        self._result = [] if row is None else [(row.seller_id_hex,)]

    def _update_product_thumbnail_ref(self, thumbnail_ref: str, product_id: str) -> None:
        # Task 100 — upload_product_thumbnail()'s post-upload write.
        # Silently a no-op for an unknown id, matching a real
        # zero-row-affected UPDATE — the real endpoint only ever calls
        # this after its own existence check already succeeded, so
        # that path isn't reachable in practice, but this fake doesn't
        # need to raise to mirror that.
        row = self._store.products.get(product_id)
        if row is not None:
            row.thumbnail_ref = thumbnail_ref
