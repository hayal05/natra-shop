...
Task 98. Extends the suite with the Object Storage client wrapper +
health check (Tasks 90-91) — see `test_object_storage.py` and the new
`GET /health/object-storage` tests in `test_health_and_startup.py`.
New fake: `fake_oci.py`, mirroring `fake_oracle.py`'s pattern for the
`oci` SDK instead of `oracledb`. First of Phase 8's four test-writing
tasks (98-101).
